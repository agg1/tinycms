...
Relying on [OpenSource Software](https://en.wikipedia.org/wiki/Open-source_software) with permissive licensing is a necessitity but **insufficient for full compliance**, and **vendor-neutral** standards must be strictly distinguished from vendor specifications[^rust-reference], for example:

- C language following ANSI/ISO C89/90 standard had proven decent, somewhat stable for >35years.
...
- [Compatibility](https://en.wikipedia.org/wiki/Computer_compatibility) with different toolchains is prioritized over
